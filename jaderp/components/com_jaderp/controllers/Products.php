<?php
/**
 * Products Controller for Jaderp Component
 * 
...
class JaderpControllerProducts extends JaderpController
{
	/**
	...
	function save()
	{
		...
		{
			if ($result->msg == '')
				$msg = JText::sprintf( 'Successfully Saved Product %s', $post['rsoc']);
			else
				$msg = $result->msg;
			$table =& JTable::getInstance('products', 'Table');
		...
		else 
		{
			...
			if ($menuid > 0)
				...
				$menulink = '';
			$this->setRedirect( 'index.php?option=com_jaderp&func=Products&task='.$task.'&cid='. $post['id'].$menulink, $msg );
			return ;
		}
		...
		switch ( $this->getTask() )
		{
			...
			default:
				...
				$this->setRedirect( 'index.php?option=com_jaderp&func=Products&task=manage', $msg );
				break;
		}
	...
	function remove()
	{
		...
		if (count($cid) < 1) {
			$msg =  JText::_('Select a product to delete');
			$this->setRedirect( 'index.php?option=com_jaderp&func=Products&task=manage', $msg, "error");
		}
		...
		/*$query = 'DELETE a, b, c FROM #__jaderp_products AS a INNER JOIN #__jaderp_supplier_contact AS b ON a.id = b.supplier_id '.
   				'INNER JOIN #__jaderp_supplier_bank AS c ON a.id = c.supplier_id '.
				'WHERE a.id IN ( '. $cids. ' )';*/
				
		$query = 'DELETE FROM #__jaderp_products '.
				'WHERE id IN ( '. $cids. ' )';
	...
	function publish()
	{
		...
		$this->setRedirect( 'index.php?option=com_jaderp&func=Products&task=manage' );
		...
		$this->setMessage( JText::sprintf( $publish ? 'PRODUCTS_TO_PRESENT' : 'PRODUCTS_TO_ABSENT', $n ) );
	}	
	/**
	...
	function cancel()
	{
		...
		if ($id)
		{
			if (!$JAdERPTool->CheckInOut('Products', $id))
			{
				$msg= JText::_( 'CANT_CHECKIN_ALERT' ) ;
			}			
		}	

			...
			<fieldset id="contactfields" style="display: block">
			  ...
			  <fieldset id="contactfields" style="display: block">
				  ...
				  <div class="FormElements">
				    ...
				    <label for="suppliername"><?php echo JText::_('NAME'); ?>:</label>
				    <br />
				...
				<?php if ($this->contactAccesses['image']['task']) { ?> 				  
				  <div class="FormElements">
				    <label for="photo"><?php echo JText::_('PHOTO'); ?>:</label>
				    <br />
				    <input name="photo[]" type="text" id="photo" size="25" />
				    <input type="button" id="browse"  value="<?php echo JText::_('BROWSE'); ?>"  /> 
				  </div>
				 <?php } ?>
				<?php if ($this->contactAccesses['country']['task']) { ?>
				 <div class="FormElements">
				  <label for="supplierCountry"><?php echo JText::_('COUNTRY'); ?>:</label>
				    <br />
				...
				<?php if ($this->contactAccesses['address']['task']) { ?>
				  <div class="FormElements">
				    <label for="supplieraddress"><?php echo JText::_('ADDRESS'); ?>:</label>
				    <br />
				...
				<?php if ($this->contactAccesses['suburb']['task']) { ?>  
				  <div class="FormElements">
				    <label for="supplieruburb"><?php echo JText::_('SUBURB'); ?>:</label>
				    <br />
				   <input name="suburb[]" type="text" id="suppliersuburb" />
				  </div>
				<?php } ?>		  
				<?php if ($this->contactAccesses['state']['task']) { ?>
				  <div class="FormElements">
				    <label for="supplierstate"><?php echo JText::_('STATE'); ?>:</label>
				    <br />
				    <input name="state[]" type="text" id="supplierstate" />
				  </div>
				<?php } ?>
				<?php if ($this->contactAccesses['postcode']['task']) { ?>
				  <div class="FormElements">
				    <label for="postcode"><?php echo JText::_('POSTAL_CODE'); ?>:</label>
				    <br />
				...
				<?php } ?>
				 ...
				 <?php if ($this->contactAccesses['webpage']['task']) { ?> 				  		  
				  <div class="FormElements">
				    <label for="webpage"><?php echo JText::_('URL_ADDRESS'); ?>:</label>
				    <br />
				    <input name="webpage[]" type="text" id="webpage" size="50" />
				  </div>
				 <?php } ?>
			  </fieldset>
		</div>
...
<form id="adminForm" name="adminForm"  method="POST" action="index.php">
  <div id="FormElementsGroup">
    <fieldset>
      <legend><?php echo JText::_('SUPPLIER_INFOS'); ?></legend>
      <!--Supplier Code-->
	...
	<?php if ($this->supplierAccesses['bank_name']['task']) { ?> 
      <div class="FormElements">
        <label for="supplierBank"><?php echo JText::_('BANK_NAME'); ?>:</label>
        <br />
        <input name="bank_name" type="text" id="supplierBank" size="40" />
      </div>
    <?php } ?>
	<?php if ($this->supplierAccesses['bank_address']['task']) { ?> 
      <div class="FormElements">
        <label for="supplierBankAddress"><?php echo JText::_('BANK_ADDRESS'); ?>:</label>
        <br />
        <textarea name="bank_address" type="text" id="supplierBankAddress" cols="25" rows="4" /></textarea>
      </div>
    <?php } ?>
...
?>
<input type="hidden" name="func" value="Suppliers" />
</form>
